...
import ModalBndesOld from '../../components/modalBndesOld'
...
const BndesLeadsDetailsOld = () => {
  ...
  const fetchData = useCallback(async () => {
    const response = await apiFinmarcBndesOld.get(`/proposals/${id}`);
    const { data } = response;
    const newProposal = {
      ...data.data,
    }
      ...
      <div className="intro-y box px-5 pt-5 mt-5">
        <div className="flex flex-col lg:flex-row border-b border-slate-200/60 dark:border-darkmode-400 pb-5 -mx-5">
          ...
          {showModalSendProposal && (
              <ModalBndesOld showModalSendProposal={showModalSendProposal}
...
export default BndesLeadsDetailsOld;
