...
export default function ModalBndesOld({ showModalSendProposal, setShowModalSendProposal, proposals , id}: Props) {
  ...
  const onSubmit: SubmitHandler<BNDESProposal> = async (data: BNDESProposal) => { 
    try {
      ...
      await apiFinmarcBndesOld.post("/proposals/send", dataSend);
